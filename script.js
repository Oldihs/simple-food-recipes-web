...
// Event Listenrs for search and input (When press enter)
...
function searchMeal(){
	...
	fetch(apiUrl + userInput).then((response) => response.json()).then((data) => {
		...
		searchContainer.getElementsByClassName.opacity = '0';
		searchContainer.getElementsByClassName.display = 'none';
	})

	.catch(() => {
		searchContainer.getElementsByClassName.opacity = '1';
		searchContainer.getElementsByClassName.display = 'grid';
...
function hideRecipe(){
	...
	const view_recipe_appear= document.getElementById("show-recipe");
	view_recipe_appear.style.display = "block";
}
...
function showRecipe(){
	...
	const view_recipe_disappear= document.getElementById("show-recipe");
	view_recipe_disappear.style.display = "none";
}